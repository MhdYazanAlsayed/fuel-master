...
The authentication token is set in a secure HTTP-only cookie named `access_token`.
...
4. **User Status Check**: Use `GET /api/auth/check` to determine the user's current status. This endpoint reads information from the JWT token claims (no database queries), making it fast and efficient. The frontend should call this endpoint after login to determine if the user needs to:
   ...
   **Note**: The status information reflects the state at the time of authentication. If status changes occur (e.g., subscription expires), users must re-authenticate to get updated information.
