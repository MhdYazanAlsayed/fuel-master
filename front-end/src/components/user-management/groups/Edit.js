...
const Edit = () => {
  if (!_roleManager.check(Permissions.GroupsEdit))
  ...
  const handleGetDetailsAsync = async () => {
    const response = await _groupService.detailsAsync(id);
    if (!response) return;
  ...
  const handleOnSubmitAsync = async e => {
    ...
    const response = await _groupService.editAsync(id, formData);
  ...
  return (
    <FormCard
      header={_languageService.resources.editGroup}
