...
const Edit = () => {
  if (!_roleManager.check(Permissions.ZonesEdit))
  ...
  const handleGetDetailsAsync = async () => {
    ...
    const response = await _zoneService.detailsAsync(id);
    if (!response) return;
  ...
  const handleOnSubmitAsync = async e => {
    ...
    const response = await _zoneService.editAsync(id, formData);
  ...
  return (
    <FormCard
      header={_languageService.resources.editZone}
