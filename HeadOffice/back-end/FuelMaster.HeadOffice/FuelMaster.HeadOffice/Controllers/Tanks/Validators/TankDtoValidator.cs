    ...
    public class TankDtoValidator : AbstractValidator<TankDto>
    {
        public TankDtoValidator()
        {
            ...
            RuleFor(x => x.CurrentLevel)
                .GreaterThan(0).WithMessage("Current level must be greater than 0")
                .LessThanOrEqualTo(x => x.MaxLimit).WithMessage("Current level must be less than or equal to max limit")
                .GreaterThanOrEqualTo(x => x.MinLimit).WithMessage("Current level must be greater than or equal to min limit");

            RuleFor(x => x.CurrentVolume)
                .GreaterThan(0).WithMessage("Current volume must be greater than 0")
                .LessThanOrEqualTo(x => x.MaxLimit).WithMessage("Current volume must be less than or equal to max limit")
                .GreaterThanOrEqualTo(x => x.MinLimit).WithMessage("Current volume must be greater than or equal to min limit");
        }
    }
}
