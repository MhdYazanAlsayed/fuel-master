...
namespace FuelMaster.HeadOffice.Application.Services.Implementations.StationConfigurations.NozzleService;
...
public class NozzleService : INozzleService
{
    ...
    // This will not filter data because its used on PTS operations (Transactions & others).
    ...
    public async Task<IEnumerable<NozzleResult>> GetAllAsync(GetNozzleDto dto)
    {
        try
        {
            ...
            var nozzles = Filter(await GetCachedNozzlesAsync());
